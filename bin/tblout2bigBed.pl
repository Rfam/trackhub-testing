...
#skip lines beginning #
	unless ($_ =~ /^#/){
		...
		if ($data[9] eq '+'){
			print BED "$data[0]\t$data[7]\t$data[8]\t$data[2]\t$bit_score\t$data[9]\n";
		} elsif ($data[9] eq '-'){
			print BED "$data[0]\t$data[8]\t$data[7]\t$data[2]\t$bit_score\t$data[9]\n";
		} else {
...
system("fetchChromSizes $db > chrom.sizes") and die "Could not create chrom.sizes for $db $!\n";


#use bedToBigBed to convert BED to bigBed
my $bigbedfile = $infile . ".bb";
system ("bedToBigBed $sortedfile chrom.sizes $bigbedfile") and die "Could not convert BED to bigBed $!\n";
